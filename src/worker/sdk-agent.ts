...
const findClaudeExecutable = (): string | undefined => {
  ...
  for (const loc of locations) {
    const check = fromTry(() => Bun.file(loc).size > 0);
          ...
          if (result.ok) {
            ...
            if (chromaSync) {
              fromPromise(
                ...
                if (!syncOuter.ok) {
                  logError(`ChromaDB sync error: ${syncOuter.error.message}`);
                } else if (!syncOuter.value.ok) {
                  logError(
                    `ChromaDB sync failed: ${syncOuter.value.error.message}`,
                  );
                } else {
          ...
          if (result.ok) {
            ...
            if (chromaSync) {
              fromPromise(
                ...
                if (!syncOuter.ok) {
                  logError(
                    `ChromaDB summary sync error: ${syncOuter.error.message}`,
                  );
                } else if (!syncOuter.value.ok) {
                  logError(
                    `ChromaDB summary sync failed: ${syncOuter.value.error.message}`,
                  );
                } else {
